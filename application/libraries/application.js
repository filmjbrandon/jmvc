function writeDebug() {
  for (var idx = 0; idx < arguments.length; idx++) {
    $('#outputblock').append('<pre>' + var_dump(arguments[idx]) + "\n");
  }
}

// writes a link in the debug div that clears debug output
function clearDebug() {
  $("#outputblock").html("<a href='#' onclick='clearDebug(); return false;'>clear</a><br>");
}
...
function test(desc,input,expected) {
  ...
  return desc + " not a match";
}

function isa(desc,input,isa) {
  if (typeof(input) === isa) {
    return desc + " is a " + isa + " true";
  }
